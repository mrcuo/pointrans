...
The approved v1.1 artwork lives in `Pointrans_Logo_Design_Files/`. Release builds run `scripts/generate_brand_assets.py`: the supplied opaque black 1024 software-icon canvas with the white symbol produces every AppIcon size, while the supplied SVG symbol and horizontal lockup are copied losslessly into the asset catalog for the menu bar, onboarding, and control center. The generator rejects the light-canvas icon variant so a wrong source cannot silently ship. Runtime code does not maintain a separate approximation of the logo.
