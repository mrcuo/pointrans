...
- AppKit delegate 由进程级生命周期对象强持有，在加载词典、模型和窗口前同步创建唯一 `NSStatusItem`；引导期间显示 Symbol + `Pointrans` 文字，完成后收为紧凑 Symbol。
...
- 引导窗口提供标准红色关闭按钮、显式退出按钮和 Command-Q；所有退出路径统一停止 event tap、悬停、AX/OCR、基础翻译、设备 AI、Worker 请求和窗口任务，并显式移除状态项。

### 强制首次引导

- 使用版本化 onboarding v4，旧 `didCompleteOnboarding`、v1、v2 或 v3 不能跳过重做后的真实浮层流程；v2 独立隐私页状态会迁移到一体化体验。
- 五阶段流程已实现：认识产品、辅助功能、屏幕录制、自动语言能力、一体化强制真实体验。独立云端路由页已经删除。
- 双权限与双向 Apple Translation 能力均为硬门槛；系统权限需重启时保存当前阶段并阻止继续。
- 强制体验把 `breakthrough` 渲染为始终可见的独立目标控件，并直接向取词控制器提供其精确语义范围，不再让 AX/OCR 读取自家界面；引导窗口只显示当前唯一动作。
- 真实翻译由正式 Preview 浮层显示，语境按钮将其转为正式 Pinned；真实语境成功后引导自动关闭而 Pinned 结果继续保留，不存在页内假结果或额外完成按钮。
...
- App/Core/UI Tests：完整编译成功；进程级 UI 测试已验证引导窗口、菜单栏标题、标准关闭、显式退出、退出后进程终止，以及最终阶段没有假结果页。
- Release：arm64/x86_64 静态分析成功。
- DMG：2.0.0（216）；严格签名、架构、现代图标、词典、挂载复制和无调试 entitlement 由 `package.sh` 全部验证。
...
生产 Worker 2.0.0 已通过 `/health`、`/version`、匿名安装注册和真实语境请求的完整契约探测。

结论：计划中的源码重构、自动化覆盖、生产 Worker 和本地安装候选已完成；公开发布状态仍需用户真实安装矩阵与 Developer ID/公证门槛通过。
